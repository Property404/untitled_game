...
template <typename T> class Clamped {
    T _val{};
    T _min{};
    T _max{};

    public:
    Clamped(T val, T min, T max) : _val(val), _min(min), _max(max) {
        assert(val >= min);
        assert(val <= max);
    }

    T inner() const { return this->_val; }
    ...
    Clamped<T> operator-=(T val) { return (*this = *this - val); }
