...
#define REQUIRE_EQUALS(value1, value2)                                                             \
    do {                                                                                           \
        __self->assert_equals("" #value1, "" #value2, value1, value2);                             \
    } while (0)
#define REQUIRE(condition)                                                                         \
    { __self->assert_bool("Assert failed: '" #condition, condition); }
...
class TestCase final {
    ...
    void assert_equals_inner(const char* type1, const char* type2, T value1, U value2) {
        if (value1 != value2) {
            std::cerr << "Assert failed: " << value1 << "(" << type1 << ")" << " != " << value2
                      << "(" << type2 << ")" << std::endl;
            std::exit(1);
        } else {
    ...
    void assert_equals(const char* type1, const char* type2, T value1, U value2) {
        this->assert_equals_inner<std::remove_reference_t<T>, std::remove_reference_t<U>>(
